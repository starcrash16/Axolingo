...
class BotonMenuPrincipalAzul : AppCompatButton {
    ...
    private fun init() {
        // Establecer el texto por defecto si no se ha definido en XML
        if (text.isNullOrEmpty()) {
            text = "BOTÓN"
        }

        // Crear el drawable para la sombra (azul oscuro)
        val shadowDrawable = GradientDrawable().apply {
            shape = GradientDrawable.RECTANGLE
            cornerRadius = 50f // Radio de las esquinas
            setColor(Color.parseColor("#3873b1")) // Color de sombra
        }
        ...
        // Crear el drawable principal del botón (verde azulado)
        val mainButtonDrawable = GradientDrawable().apply {
            shape = GradientDrawable.RECTANGLE
            cornerRadius = 45f // Un poco menos para que el borde blanco se vea
            setColor(Color.parseColor("#4dc2a3")) // Color principal del botón
        }
        ...
        setTextColor(Color.parseColor("#3873b1")) // Color del texto
        try {
